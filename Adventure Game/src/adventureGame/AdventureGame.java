	...
	public static void main(String[] args) throws InterruptedException {
		...
		do {
			...
			try{
				...
				breathing.open(AudioSystem.getAudioInputStream(new File("AdentureGameAudio/breathing.wav")));
			...
			try{
				...
				siren.open(AudioSystem.getAudioInputStream(new File("AdentureGameAudio/siren.wav")));
			...
			if (run.equals("run")) {
				...
				c.drawImage(lasersecurity, 40, 150,40,150, null);
				...
				if (up.equals("up")) {
					...
					if (phone.equals("yes")) {
						...
						c.drawImage(outside, 0, 0, null);
						c.println("The helicopter guy called you and you escape the museum. Mission accomplished! You successfully stole:"); //end of mission
						for (int i = 0; i< items.length;i++)
						{
							c.println(items[i]);
						}
					...
					keyboard.loop(Clip.LOOP_CONTINUOUSLY);
					c.println("You go downstairs to the basement but it is protected by a keypad. Can you guess the code? \n30/5+4*(-2)=");
					double math;
					try{
							...
							if(cell.equals("stay")){
								...
								if (chance < 0.5){
									...
									if(!found){
										...
										c.println("You try to fight the guards with your fists due to the lack of weapons you have and fail. You almost stole:");
										for (int i = 0; i< items.length;i++)
										{
											c.println(items[i]);
										}
					...
					c.drawImage(handcuffs, 0, 0, null);
					c.println("You're out of guesses and set off the alarms. You failed the mission. You almost stole:"); //end of mission
					for (int i = 0; i< items.length;i++)
					{
						c.println(items[i]);
					}
