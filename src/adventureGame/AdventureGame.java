	...
	public static void main(String[] args) throws InterruptedException {
		...
		do {
			...
			try {
				...
				attack.open(AudioSystem.getAudioInputStream(new File("attack.wav")));
			...
			try {
				...
				doorknob.open(AudioSystem.getAudioInputStream(new File("doorknob.wav")));
			...
			try {
				...
				breathing.open(AudioSystem.getAudioInputStream(new File("breathing.wav")));
			...
			try {
				...
				helicopter.open(AudioSystem.getAudioInputStream(new File("helicopter.wav")));
			...
			try {
				...
				siren.open(AudioSystem.getAudioInputStream(new File("siren.wav")));
			...
			try {
				...
				punches.open(AudioSystem.getAudioInputStream(new File("punches.wav")));
			...
			for (int i = 0; i < items.length; i++) {
				items[i] = c.readLine().toLowerCase(); // saves the items that
														// they stole into an
														// array
				while (!(items[i].equals("bedazzled gun") || items[i].equals("royal crown")
					...
					c.println("Invalid answer please answer again"); // while
																		// loop
																		// for
																		// spell-check
			...
			if (run.equals("run")) {
				...
				if (pass.equals("pass")) {
					...
					double num = Math.random(); // random number generator 50%
					...
					if (num < 0.5) {
						...
						if (trip.equals("trip")) {
							...
							if (right.equals("right")) {
								...
								c.println("You run into the police and get caught. You failed the mission."); // end
																												// of
																												// mission
								...
								c.drawImage(outside, 0, 0, null);
								c.println(
										"You manage to escape the museum unharmed. Mission accomplished! You successfully stole:"); // end
																																	// of
																																	// mission
								for (int i = 0; i < items.length; i++) {
									c.println(items[i]);
							...
							c.drawImage(handcuffs, 0, 0, null);
							c.println("The guards see you and catch you. Mission unsuccessful.");// end
																									// of
																									// mission
							c.println("Although unsuccessful, you almost stole:");
							for (int i = 0; i < items.length; i++) {
						...
						c.drawImage(handcuffs, 0, 0, null);
						c.println(
								"You didnt make it though the laser security. The guards caught you. Mission unsuccessful."); // end
																																// of
																																// mission
						c.println("Although unsuccessful, you almost stole:");
						for (int i = 0; i < items.length; i++) {
					...
					while (counter < 3) {
						...
						if (answer.equals("age")) {
							c.drawImage(outside, 0, 0, null);
							c.println(
									"The lasers have deactivated and you safely escape the museum. Mission accomplished!"); // end
																															// of
																															// mission
							for (int i = 0; i < items.length; i++) {
								c.println(items[i]);
				...
				if (up.equals("up")) {
					...
					c.drawImage(phonecall, 0, 0, null);
					c.println("Your phone begins to ring but it's an unknown number. Do you pick up? (yes/no)"); // ringtone
																													// sound?
					...
					if (phone.equals("yes")) {
						...
						c.drawImage(handcuffs, 0, 0, null);
						c.println("The cops were calling you and you got busted! You failed the mission."); // end
																											// of
																											// mission
						c.println("Although unsuccessful, you almost stole:");
						for (int i = 0; i < items.length; i++) {
						...
						c.drawImage(outside, 0, 0, null);
						c.println(
								"The helicopter guy called you and you escape the museum. Mission accomplished! You successfully stole:"); // end
																																			// of
																																			// mission
						for (int i = 0; i < items.length; i++) {
							c.println(items[i]);
					...
					while (counter1 < 3) {
						...
						if (math == 0) {
							...
							if (cell.equals("stay")) {
								...
								c.drawImage(security5, 0, 0, null);
								c.println(
								...
								double chance = Math.random(); // random number
																// generator 50%
																// chance of
																// getting
																// caught
								if (chance < 0.5) {
									...
									c.drawImage(outside, 0, 0, null);
									c.println(
											"You manage to escape the museum safely on the helicopter. Mission accomplished! You successfully stole:"); // end
																																						// of
																																						// mission
									for (int i = 0; i < items.length; i++) {
										c.println(items[i]);
									...
									for (int i = 0; i < items.length; i++) {
										if (items[i].equals("golden dagger") || items[i].equals("bedazzled gun")) {
											c.println("You manage to win the fight with " + items[i]); // what
																										// if
																										// they
																										// had
																										// both
																										...
																										// then?
											c.drawImage(outside, 0, 0, null);
											c.println("You successfully escape the museum! You managed to steal:"); // end
																													// of
																													// mission
											c.println(items[0]);
											c.println(items[1]);
							...
							math = c.readDouble(); // use this line or the
													// try-catch loop??
							// add keyboard sound here again???
						}
					...
					c.drawImage(handcuffs, 0, 0, null);
					c.println(
							"You're out of guesses and set off the alarms. You failed the mission. You almost stole:"); // end
																														// of
																														// mission
					for (int i = 0; i < items.length; i++) {
						c.println(items[i]);
			...
			c.print("Do you want to play again?(y/n)"); // play again or restart
														// the game
			// how to make all the endings play this play again message?
